...
func update_boss_behavior():
				# CORRIGIDO BUG-016: Verificar is_instance_valid além de not boss
				if not boss or not is_instance_valid(boss) or current_phase != PhaseState.PHASE_1_ESCAPE:
								return

# ---------------------------------------------------------------

func _on_phase_transition_entered(body):
				print("DEBUG: Corpo entrou na área de transição: ", body.name if body else "null")
				print("DEBUG: É o player? ", body == player)
				print("DEBUG: Fase atual: ", current_phase)
				if body == player and current_phase == PhaseState.PHASE_1_ESCAPE:
								print("DEBUG: Iniciando transição para Fase 2!")
...
func start_phase_2():
				...
				# Trocar para música de boss
				if AudioManager:
								AudioManager.play_music("boss", 0.5)
...
func _on_boss_defeated():
				...
				# Aguardar 2 segundos e mostrar tela de vitória
