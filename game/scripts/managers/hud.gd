...
func _ready():
				...
				if hp_bar:
								hp_bar.modulate = Color(1.0, 0.2, 0.2)  # Vermelho para HP
				if mp_bar:
								mp_bar.modulate = Color(0.2, 0.5, 1.0)  # Azul para MP
				if stamina_bar:
								stamina_bar.modulate = Color(1.0, 0.8, 0.0)  # Amarelo/Dourado para Stamina
				...
				if not player:
								print("⚠️ HUD: Jogador não encontrado!")
...
func update_hud():
				...
				if mp_label:
								mp_label.text = "MP: %d/%d" % [int(player.current_mp), int(player.max_mp)]
				
				# Atualizar Stamina
				if stamina_bar:
								stamina_bar.value = (player.current_stamina / player.max_stamina) * 100
								# Mudar cor se stamina baixa
								if player.stamina_penalty_active:
												stamina_bar.modulate = Color(1.0, 0.5, 0.0)  # Laranja quando penalizada
								else:
												stamina_bar.modulate = Color(1.0, 0.8, 0.0)  # Amarelo/Dourado normal
				if stamina_label:
								stamina_label.text = "Stamina: %d/%d" % [int(player.current_stamina), int(player.max_stamina)]
